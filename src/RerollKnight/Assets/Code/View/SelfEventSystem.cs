...
namespace Code
{
	public sealed class SelfAddedOrRemovedEventSystem<TScope, TComponent> : ReactiveSystem<Entity<TScope>>
		where TScope : IScope
		where TComponent : class, IComponent, IEvent<Self>, new()
	{
		private readonly List<IListener<TScope, TComponent>> _listenerBuffer;

		public SelfAddedOrRemovedEventSystem(Contexts contexts)
